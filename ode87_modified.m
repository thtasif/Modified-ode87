function [tout,xout,nsteps,nfailed,nfevals] = ode87_modified(odefun,tspan,x0,options,varargin)
%This file has been modified by Tahsinul Haque Tasif.
%This modified version of the code can show the total number of steps,
%number of failed steps and number of function evaluations in an
%integration.
%
...
while (t < tfinal) & (h >= hmin)
    ...
    nfevals = nfevals + 1; % ----> Added line
    
    for j = 1: 12
        f(:,j+1) = feval(odefun, t+c_i(j)*h, x+h*f*a_i_j(:,j), varargin{:});
    end
    nfevals = nfevals + 12; % ----> Added line
